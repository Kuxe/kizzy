...
// Takes an array of chars and splits it into an array of strings using space as
// delimiter
void split(char strargv[], char **argv, const char *delimiter) {
  int i = 0;
  char *token = strtok(strargv, delimiter);
  while (token != NULL) {
    argv[i++] = token;
...
int ProcessLauncher::launch(const std::string &execPath) {
  ...
  // If the execPath points to an executable within a directory (such as
  // /home/username/myprogram) then set workingdirectory as (/home/username/) if
  // the execPath points to an executable in path (ie execPath = myprogram) then
  // do nothing
  const size_t workingDirectoryPos = execPath.find_last_of("/");
  if (workingDirectoryPos != std::string::npos) {
    const std::string workingDirectory =
        execPath.substr(0, workingDirectoryPos);
  ...
  if (argsPos != std::string::npos) {
    ...
    split(strargv, buffer, " ");

    // For each word in strargv (ie execpath), let buffer[i] hold a pointer to
    // beginning of that word execvp wants a char** where each char[i] holds an
    // argument
    int i = 0;
    ...
    // The variable i will be >= 1 due to the program name, so if i >= 2 then
    // there have been args
    if (i >= 2) {
      buffer[i + 1] = NULL;
      ...
      if (execlp(buffer[0], nullptr) == -1) {
        printf(
            "Couldn't launch executable on %s (with args that was ignored)\n",
            execPath.c_str());
        return STATUS_CODE::EXEC_FAILED;
      }
    }
