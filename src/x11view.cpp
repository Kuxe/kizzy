...
void X11View::redraw() {
  ...
  const auto newWindowHeight =
      TOP_PADDING + DIFF_Y_OFFSET * searchResults->size();
  ...
  for (const ApplicationMatching &applicationMatching : *searchResults) {
    itemNum == selectedItem ? highlightItemNumber(itemNum)
                            : darkenItemNumber(itemNum);
    itemNum += 1;
  }

  // Clear away old results
  XClearArea(display, window, 0, TOP_PADDING + DIFF_Y_OFFSET * (itemNum - 1), 0,
             0, false);
...
void X11View::highlightItemNumber(int itemNumber) const {
  XClearArea(display, window, 0,
             2 + TOP_PADDING + DIFF_Y_OFFSET * (itemNumber - 1), 0, 12, false);
  ...
  // painters algorithm!
  const std::string &selectedAppnameArrows =
      ">" + (*searchResults)[itemNumber].getApplication()->getName() + "<";
  XSetForeground(display, gc, arrowColor.pixel);
  XDrawString(display, window, gc, 4, TOP_PADDING + DIFF_Y_OFFSET * itemNumber,
              selectedAppnameArrows.c_str(), selectedAppnameArrows.length());

  // Draw only string
  const std::string &selectedAppname =
      (*searchResults)[itemNumber].getApplication()->getName();
  XSetForeground(display, gc, selectedColor.pixel);
  XDrawString(display, window, gc, 10, TOP_PADDING + DIFF_Y_OFFSET * itemNumber,
              selectedAppname.c_str(), selectedAppname.length());
...
void X11View::darkenItemNumber(int itemNumber) const {
  ...
  XDrawString(display, window, gc, PADDING,
              TOP_PADDING + DIFF_Y_OFFSET * itemNumber, appname.c_str(),
              appname.length());
}
...
X11View::X11View(int &status)
    : // Doesn't actually need to removeLastCharacter, but pointer to
      // searchresults can be set this way :)
  ...
  backgroundColor.red = 0x2121;
  backgroundColor.green = 0x2121;
  backgroundColor.blue = 0x2121;
  backgroundColor.flags = RGB_FLAGS;
  XAllocColor(display, colormap, &backgroundColor);

  foregroundColor.red = 0x3B3B;
  foregroundColor.green = 0x5353;
  foregroundColor.blue = 0x8484;
  foregroundColor.flags = RGB_FLAGS;
  XAllocColor(display, colormap, &foregroundColor);

  selectedColor.red = 0xCCCC;
  selectedColor.green = 0x6C6C;
  selectedColor.blue = 0xA5A5;
  selectedColor.flags = RGB_FLAGS;
  XAllocColor(display, colormap, &selectedColor);

  arrowColor.red = 0x7777;
  arrowColor.green = 0x8989;
  arrowColor.blue = 0xB8B8;
  arrowColor.flags = RGB_FLAGS;
  XAllocColor(display, colormap, &arrowColor);

  searchColor.red = 0xFFFF;
  searchColor.green = 0xFFFF;
  searchColor.blue = 0xFFFF;
  ...
  const short x = (screenWidth / 2) - (WINDOW_WIDTH / 2),
              y = (screenHeight / 2) - (windowHeight / 2);

  window =
      XCreateWindow(display, rootWindow, x, y, WINDOW_WIDTH, windowHeight, 0,
  ...
  // I honestly don't know. Got it off internet. For proper, I think, closing of
  // X11-window
  ...
  while (true) {
    ...
    case KeyPress: {
      ...
      case XK_Return: {
        if (!searchResults->empty()) {
          const Application *bestMatch =
              (*searchResults)[selectedItem].getApplication();
          status = ProcessLauncher::launch(
              (*searchResults)[selectedItem].getApplication()->getExec());
        }
        return;
      } break;
      ...
      case XK_Tab: {
        darkenItemNumber(selectedItem); // Darken the current selection

        if (searchResults->empty()) {
          selectedItem = 0; // Handle empty searchResults
        } else {
          selectedItem = (selectedItem + 1) % searchResults->size();
        }

        highlightItemNumber(selectedItem); // Highlight the new selection
        paintPadding();
      } break;
